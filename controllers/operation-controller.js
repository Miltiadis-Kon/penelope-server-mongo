...
const getOperationByID = (req, res, next) => { // get a task by id
  const operationID = req.params.operationID; // get the task id from the request params
  const Operation = DUMMY_operations.find((p) => { // find the task with the id provided in the request
    return p.id === operationID; // return the task
  });
  if (!Operation) {
    throw new HttpError("Could not find an operationID for the provided id.", 404); // if the task does not exist, throw an error
  }
  res.json({ Operation});
};
//Search task by UserId.
const getOperations = (req, res, next) => { // get a task by user id
  const operations= DUMMY_operations;
  if (!operations ||operations.length==0) { // if the task does not exist, throw an error
    throw new HttpError("Could not find a task for the provided id.", 404);
  }
  res.json({ operations }); // return the task to the client
};
//Search task by UserId.
const getOperationsInMethodID = (req, res, next) => { // get a task by user id
  const operationID = req.params.uid; // get the user id from the request params
  const operations = DUMMY_operations.filter((p) => { // find the task with the user id provided in the request
...
const createOperation = (req, res, next) => { // create a new task
  const { title, description, creator, tags } = req.body; // get the data from the request body
  const createOperation = { // create a new task object
    id: uuid(),
    title,
    description,
    creator,
...
const updateOperation = (req, res, next) => {    
   ...
   const updatedOperation = { ...DUMMY_operations.find((p) => p.id === operationID) }; //find the task to update and copy it to a new object called updatedOperation 
   const OperationIndex = DUMMY_operations.findIndex((p) => p.id === operationID); // find the index of the task to update in the DUMMY_operations array 
...
const deleteOperation= (req, res, next) => {
  ...
  if (!DUMMY_operations.find((p) => p.id === operationID)) { //check if the task exists
